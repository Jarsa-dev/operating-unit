...
class TestInvoiceOperatingUnit(test_ou.TestAccountOperatingUnit):
    ...
    def test_payment_from_two_invoices_with_group_payment(self):
        """ Create two invoices of different OU and payment from a third OU
        using Group Payment.
        """
        ...
        register_payments.create_payments()
        payment = self.payment_model.search([], order="id desc", limit=1)
        self.assertEqual(len(payment.invoice_ids), 2)
        self.assertEqual(len(payment.move_line_ids), 3)
        self.assertEqual(len(payment.move_line_ids.mapped("operating_unit_id")), 3)
